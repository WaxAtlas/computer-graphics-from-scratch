...
fn canvas_to_viewport(x: f32, y: f32) -> Vector {
    let a = x * VIEWPORT_SIZE / CANVAS.width;
    let b = y * VIEWPORT_SIZE / CANVAS.height;
    println!("{}, {}, {}, {}", CANVAS.width, CANVAS.height, a, b);
...
fn trace_ray(origin: Vector, direction: Vector, t_min: f32, t_max: f32) -> Option<Color> {
    ...
    for sphere in SPHERES.iter() {
        ...
        } else if (t_min < ts[1]) && (ts[1] < t_max) && (ts[0] < closest_t) {
            closest_t = ts[1];
            closest_sphere = Some(sphere);
        }
    }
...
fn put_pixel(x: f32, y: f32, color: &Option<Color>, file: &mut File) {
    ...
    match color {
        ...
        .ok(),
        Some(i) => writeln!(file, "{}, {}, {}", i.r, i.b, i.g).ok(),
    };
}
...
fn main() {
    ...
    writeln!(file, "P3").ok();
    writeln!(file, "{} {}", CANVAS.width, CANVAS.height).ok();
    writeln!(file, "255").ok();

    for x in (-CANVAS.width as i32 / 2)..(CANVAS.width as i32 / 2 + 1) {
        for y in (-CANVAS.height as i32 / 2)..(CANVAS.height as i32 / 2 + 1) {
